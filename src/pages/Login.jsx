      ...
      <div style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', padding:'32px 16px' }}>
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ position:'absolute', bottom:-40, left:'30%', width:150, height:150,
              borderRadius:'50%', background:'rgba(232,93,62,0.15)' }}/>
            <img src="https://images.unsplash.com/photo-1593113598332-cd288d649433?w=800&q=80"
              ...
              objectFit:'cover', mixBlendMode:'overlay', opacity:0.25 }}/>
          ...
          <div style={{ padding:'40px 36px', display:'flex', flexDirection:'column', justifyContent:'center' }}>
            ...
            <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:16 }}>
              {aba === 'cadastrar' && (
                <div>
                  <label style={{ display:'block', fontSize:11, fontWeight:700, color:'#7a5e55',
                    textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:6 }}>Nome completo</label>
              ...
              <div>
                <label style={{ display:'block', fontSize:11, fontWeight:700, color:'#7a5e55',
                  textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:6 }}>E-mail</label>
              ...
              <div>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:6 }}>
                  <label style={{ fontSize:11, fontWeight:700, color:'#7a5e55',
                    textTransform:'uppercase', letterSpacing:'0.06em' }}>Senha</label>
                  {aba === 'entrar' && (
                    <a href="#" style={{ fontSize:13, color:'#c4674e', fontWeight:600, textDecoration:'none' }}>
                      Esqueceu?
                    </a>
                  )}
                </div>
                <div style={{ position:'relative' }}>
                  ...
                  <button type="button" onClick={() => setMostrarSenha(!mostrarSenha)}
                    style={{ position:'absolute', right:14, top:'50%', transform:'translateY(-50%)',
                      background:'none', border:'none', cursor:'pointer', color:'#7a5e55', display:'flex', padding:4 }}>
              ...
              <button type="submit" style={{
                ...
                border:'none', borderRadius:9999, padding:'14px 0', fontWeight:700,
            ...
            {/* Erro */}
            {erro && (
              <div style={{ background:'#fdecea', border:'1.5px solid #f5c6c6', borderRadius:10,
                padding:'10px 14px', marginBottom:12, color:'#c0392b', fontSize:13 }}>
                {erro}
              </div>
            )}
            ...
            <button onClick={handleGoogle} disabled={loadingGoogle}
              style={{
                ...
                background: loadingGoogle ? '#f8ece0' : 'white',
                cursor: loadingGoogle ? 'not-allowed' : 'pointer',
                fontWeight:600, fontSize:15, fontFamily:'inherit', color:'#2d1f1a',
                transition:'all 0.15s',
                boxShadow: loadingGoogle ? 'none' : '0 1px 4px rgba(0,0,0,0.06)',
              }}
              onMouseEnter={e => { if (!loadingGoogle) e.currentTarget.style.borderColor = '#4285F4' }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = '#e5cfc0' }}>
              {loadingGoogle ? (
                <>
                  <div style={{ width:20, height:20, border:'2px solid #e5cfc0',
            ...
            <div style={{ display:'flex', alignItems:'center', gap:12, marginTop:24,
              paddingTop:20, borderTop:'1px solid #f0dfd0' }}>
              <div style={{ display:'flex' }}>
                {[47,12,32,15].map((i,idx) => (
                  <img key={i} src={`https://i.pravatar.cc/32?img=${i}`} alt=""
                    style={{ width:30, height:30, borderRadius:'50%', border:'2px solid white',
                      marginLeft: idx > 0 ? -8 : 0, objectFit:'cover' }}/>
                ))}
              </div>
